                    ...
                    for ip in sorted(ips):
                        if self.ip_db.find_one({'_id': str(self._ip_into_int(ip))}):
                            hosts = list(
                                ...
                                cur_sub_domain)
                            domains = list(
                                self.ip_db.find_one({'_id': str(self._ip_into_int(ip))}).get('domains')).append(
                                self.target)
                            self.ip_db.update_one({'_id': str(self._ip_into_int(ip))},
                                                  {'$set': {'hosts': list(set(hosts)), 'timestamp': int(time.time()),
                                                            'domains': list(set(domains))}})
                        else:
                            self.ip_db.save(
                                {'_id': str(self._ip_into_int(ip)), 'hosts': [cur_sub_domain], 'domains': [self.target],
                                 'ip': ip, 'timestamp': int(time.time())})
                    try:
